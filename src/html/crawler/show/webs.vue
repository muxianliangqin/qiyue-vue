      ...
      return {
        ...
        tablePage: {
          columns: [
                ...
                return h('CrawlerShowWebsExpand', {
                  ...
                  on: {
                    modify: (row) => {
                      ...
                      this.category.fields.forEach((v) => {
                        v['value'] = row[v['key']]
                      })
                    ...
                    read: (row) => {
                      if (row.newNum !== 0) {
                        this.$http.post(this.url.web.hasRead, {
                          categoryId: row.id
                        }, (response) => {
                          if (response.errorCode === this.$const.RESPONSE_SUCCESS) {
                            this.reload()
                          } else {
                            this.$Notice.error({
            ...
            {title: '网站链接', key: 'url'},
            {
              title: 'test', align: 'center',
              render: (h, params) => {
                let button = h('SystemAuth', {
                  props: {
                    permission: 'r'
                  },
                  on: {
                ...
                let modify = h('a', {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.web.fields.forEach((v) => {
                        v['value'] = params.row[v['key']]
                      })
